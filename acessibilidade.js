...
// Função para ler a tabela "responsible"
function lerTabelaResponsavel(pageSelector) {
    const page = document.querySelector(pageSelector);
    if (page) {
        // Encontrar a tabela com a classe 'table responsible'
        const tabela = page.querySelector('.table.responsible');
        if (tabela) {
            const rows = tabela.querySelectorAll("tr");
            rows.forEach((row, index) => {
                const cells = row.querySelectorAll("td, th");
                cells.forEach(cell => {
                    textoParaLer += ` ${cell.textContent.trim()}`;
                });
